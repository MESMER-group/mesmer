...
def test_distrib_init_all_default():
    ...
    assert optim_mx.maxiter == 1000 * optim_mx.n_coeffs * (np.log(optim_mx.n_coeffs) + 1)
...
def test_distrib_init():
    ...
    tests_mx = mesmer.mesmer_x.distrib_tests(expression, threshold_min_proba, boundaries_params, boundaries_coeffs)
    ...
    optim_mx = mesmer.mesmer_x.distrib_optimizer(expression, tests_mx, options_optim, options_solver)
    
    first_guess = np.array([1, 0.1])
    func_first_guess = None
    fg_mx = mesmer.mesmer_x.distrib_firstguess(expression, tests_mx, optim_mx, first_guess, func_first_guess)
...
def test_distrib_init_errors():
    ...
    with pytest.raises(ValueError, match="nan values in predictors"):
        result = fg_mx._find_fg_np(np.array([1, 2, np.nan]), np.array([1, 2, 3]), np.array([1, 1, 1])/3)

    with pytest.raises(ValueError, match="infinite values in predictors"):
        result = fg_mx._find_fg_np(np.array([1, 2, np.inf]), np.array([1, 2, 3]), np.array([1, 1, 1])/3)

    with pytest.raises(ValueError, match="nan values in target"):
        result = fg_mx._find_fg_np(np.array([1, 2, 3]), np.array([1, 2, np.nan]), np.array([1, 1, 1])/3)

    with pytest.raises(ValueError, match="infinite values in target"):
        result = fg_mx._find_fg_np(np.array([1, 2, 3]), np.array([1, 2, np.inf]), np.array([1, 1, 1])/3)
    ...
    with pytest.raises(ValueError, match="method for this fit not prepared, to avoid"):
        _ = mesmer.mesmer_x.distrib_optimizer(
            ...
            tests_mx,
            None,
            {"method_fit": "this is not a method"}
            )

...
def test_generate_fourier_series_np():
    ...
    # dummy yearly cycle
    expected = -np.sin(2*np.pi * (months) / 12) - 2* np.cos(2*np.pi * (months) / 12)
...
def test_fit_to_bic_np(coefficients):
    ...
    monthly_target = generate_fourier_series_np(coefficients, int(len(coefficients)/4), yearly_predictor, months)
    selected_order, estimated_coefficients, predictions = fit_to_bic_np(yearly_predictor, monthly_target, max_order=6)

    assert selected_order == int(len(coefficients)/4)
    np.testing.assert_allclose(estimated_coefficients[0:selected_order*4], coefficients, atol=1e-2)

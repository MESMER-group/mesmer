...
def main():
    ...
    # fit harmonic model
    harmonic_model_fit = mesmer.stats.fit_harmonic_model(tas_stacked_y.tas, tas_stacked_m.tas)

    # train power transformer
    resids_after_hm = tas_stacked_m - harmonic_model_fit.predictions
    pt_coefficients = mesmer.stats.fit_yeo_johnson_transform(resids_after_hm.tas, tas_stacked_y.tas)
    transformed_hm_resids = mesmer.stats.yeo_johnson_transform(resids_after_hm.tas, pt_coefficients, tas_stacked_y.tas)

    # fit cyclo-stationary AR(1) process
    AR1_fit = mesmer.stats.fit_auto_regression_monthly(transformed_hm_resids.transformed, time_dim="time")

    # work out covariance matrix
    geodist = mesmer.geospatial.geodist_exact(tas_stacked_y.lon, tas_stacked_y.lat)
    LOCALISATION_RADII = [1250,1500,1750,2000,2250,2500,2750,3000,3250,3500,3750,4000,4250,4500,4750,5000,5250,5500,6000,6250,6500,7000,7500,8000,8500]
    ...
    localized_ecov = mesmer.stats.find_localized_empirical_covariance_monthly(AR1_fit.residuals, weights, phi_gc_localizer, "time", 30)
    ...
    monthly_harmonic_emu = mesmer.stats.predict_harmonic_model(tas_stacked_y.tas, harmonic_model_fit.coeffs, m_time)

    # generate variability around 0 with AR(1) model
    local_variability_transformed = mesmer.stats.draw_auto_regression_monthly(AR1_fit, localized_ecov.localized_covariance, 
                                                                              time = m_time, 
    ...
    local_variability_inverted = mesmer.stats.inverse_yeo_johnson_transform(local_variability_transformed,
                                                                                pt_coefficients,
                                                                                tas_stacked_y.tas)

...
from mesmer.io import load_cmip, load_phi_gc, load_regs_ls_wgt_lon_lat,test_combination_vars
from mesmer.utils import read_form_fit_distrib, convert_dict_to_arr, extract_land, separate_hist_future
...
#==============================================================
# short preparation
if run_on_exo:os.nice(19) # Priority of this script on the server. Value in [-20,19], default at 0, higher is nicer to others
...
dir_cmip_X = {'txx':'/net/cfc/landclim1/mathause/projects/IPCC_AR6_CH11/IPCC_AR6_CH11/data/cmip6/tasmax/txx_regrid',\
...
esms = [ cfg.esms[3] ] # cfg.esms # just taking 1 esm (CanESM5) with several ensemble members for refactoring
if run_on_exo:
    esms = esms[subindex_csl*runs_per_process:(subindex_csl+1)*runs_per_process]

# preparing data
time, PRED, SUB_PRED, reg_dict, ls, wgt_g, lon, lat, land_targ, land_pred, phi_gc, ind, gp2reg, ww_reg, used_esms, dico_gps_nan = load_inputs_MESMERx( cfg, [targ, pred, sub_pred], esms )
...
tmp_pred = {scen:np.hstack([PRED[esm][scen][:,0,np.newaxis], PRED[esm][scen][:,:-1]]) for scen in PRED[esm].keys()}
...
for scen in pred_s.keys():
    ...
    predictors[-1][0]['GMT_t'] = xr.DataArray(pred_s[scen], coords={'member':np.arange(pred_s[scen].shape[0]), 'time':time_s[scen]}, dims=('member','time',))
    predictors[-1][0]['GMT_tm1'] = xr.DataArray(tmp_pred_s[scen], coords={'member':np.arange(pred_s[scen].shape[0]), 'time':time_s[scen]}, dims=('member','time',))

# preparing target
target = []
for scen in land_targ_s.keys():
    target.append( (xr.Dataset(),scen) )
    target[-1][0][targ] = xr.DataArray(land_targ_s[scen], coords={'member':np.arange(land_targ_s[scen].shape[0]), 'time':time_s[scen], 'gridpoint':np.arange(land_targ_s[scen].shape[2])}, dims=('member','time','gridpoint',))
#--------------------------------------------------------------
#--------------------------------------------------------------
...
xr_coeffs_distrib, xr_qual = xr_train_distrib(predictors=predictors, target=target, target_name=targ, expr=expr, expr_name=expr_name, option_2ndfit=False, r_gasparicohn_2ndfit=500, scores_fit=['func_optim', 'NLL', 'BIC'])

# probability integral transform: projection of the data on a standard normal distribution
transf_target = probability_integral_transform(data=target,\
                                               expr_start=expr, coeffs_start=xr_coeffs_distrib, preds_start=predictors,\
...
emus = probability_integral_transform(data=transf_emus,\
                                      expr_start="norm(loc=0, scale=1)", \
                                      expr_end=expr, coeffs_end=xr_coeffs_distrib, preds_end=preds_newscen)
#--------------------------------------------------------------
#--------------------------------------------------------------
#============================================================== 
#============================================================== 

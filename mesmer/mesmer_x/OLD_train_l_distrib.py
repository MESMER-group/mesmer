...
from math import e
from mesmer.utils import read_form_fit_distrib, eval_param_distrib, sigmoid_transf, sigmoid_backtransf
    ...
    **kwargs : additional parameters that will be fed to the class 'distrib_cov' for fit of a distribution
    ...
    for var_targ in targs:
        ...
        if set(descrip_fit.keys()) == set(tmp_list):
            ...
            raise Exception( 'Given the distribution ('+distr+'), should have information on {'+', '.join(tmp_list)+'}, but got instead {'+', '.join(list(set( descrip_fit.keys() )))+'}' )

        # setting options in special cases --> for cfg?
        if ('transfo' in descrip_fit) and (descrip_fit['transfo'] != ''):
            boundaries_coeffs = {'transfo_asymptleft':[0,np.inf], 'transfo_asymptright':[0,np.inf]}
        ...
        for param in params_list:
            ...
            for name_covar in descrip_fit[param[len('cov_'):]]:
                ...
                _data = {sc:np.repeat(np.copy(preds[var_targ][name_covar][sc])[np.newaxis,:], n_runs[sc], axis=0) for sc in preds[var_targ][name_covar]}
                tmp_preds[param].append( [name_covar, np.hstack([_data[scen][:n_runs[scen],:].flatten() for scen in common_scen]), form_fit] )
        ...
        for i_gp in np.arange( nr_gps ):
            print( 'Fitting a', distr, 'on', var_targ, ':', str(np.round(100.*(i_gp+1)/nr_gps,1)), '%', end='\r' )
            ## shaping inputs for covariated GEV (has checked before that it is Runs x Time x GridPoints)
            data_in = np.hstack( [data[scen][:n_runs[scen],:,i_gp].flatten() for scen in common_scen] )
            
            ## fitting covariated GEV:@HERE, parameters to improve the fit --> class 'distrib_cov' using '**kwargs'
            tmp_cov = distrib_cov( data=data_in, cov_preds=tmp_preds, distrib=distr, boundaries_coeffs=boundaries_coeffs, **kwargs )
            sol = tmp_cov.fit()

            ## saving
            sols.append( sol )
            if np.all(np.isnan(list(sol.values())))==False:# want to remove the case for missing points in data
        ...
        params = { kk:np.array( [sols[ii][kk] for ii in np.arange(len(sols))] ) for kk in sols[0] }
        ...
        for cov_par in params_list:
            params_out[var_targ][cov_par] = [[item[0],item[2]] for item in tmp_preds[cov_par]]
...
def transf_distrib2normal(preds, targs, params_l_distrib, threshold_sigma=6.):
    ...
    for var_targ in targs.keys():
        ...
        params_all = eval_param_distrib( param=params_l_distrib[var_targ], cov=preds[var_targ], force_scen=params_l_distrib[var_targ]['scenarios_training'] )
        
        # checking if need to do a transformation
        descrip_fit = read_form_fit_distrib( params_l_distrib[var_targ]['form_fit_distrib'] )
        ...
        for scen in params_l_distrib[var_targ]['scenarios_training']:
            ...
            ind_NoNaN = [ii for ii in range(targs[var_targ][scen].shape[2]) if ii not in params_l_distrib[var_targ]['gridpoints_nan']]
            
            if 'transfo' in descrip_fit.keys():
                left = params_l_distrib[var_targ]['parameters']['transfo_asymptleft'][ind_NoNaN]
                right = params_l_distrib[var_targ]['parameters']['transfo_asymptright'][ind_NoNaN]
                transfo = descrip_fit['transfo']
                if transfo in ['generalizedlogistic', 'generalizedalgebraic']:
                    alpha = params_l_distrib[var_targ]['parameters']['transfo_alpha']
                    data_for_PIT = sigmoid_transf( data=targs[var_targ][scen][...,ind_NoNaN], left=left, right=right, type_sigm=transfo, alpha=alpha )
                else:
                    data_for_PIT = sigmoid_transf( data=targs[var_targ][scen][...,ind_NoNaN], left=left, right=right, type_sigm=transfo )
            else:
                data_for_PIT = targs[var_targ][scen][...,ind_NoNaN]

            
            transf_inputs[var_targ][scen] = np.nan * np.ones( targs[var_targ][scen].shape )
            ...
            for i_run in np.arange( nrr ):
                ...
                if distr in ['GEV']:# distribution that have location, scale AND shape
                    if params_all[scen]['loc_all'].ndim == 3:# such a test could be avoided if using xarrays
                        tmp = distr_cdf(
                ...
                elif distr in ['gaussian']: # distribution that have location, scale BUT no shape
                    if params_all[scen]['loc_all'].ndim == 3:# such a test could be avoided if using xarrays
                        tmp = distr_cdf(
                                        x=data_for_PIT[i_run,:,:], 
                                        loc=params_all[scen]['loc_all'][i_run,...,ind_NoNaN], 
                ...
                elif distr in ['poisson']: # distribution that have location, scale BUT no shape
                    if params_all[scen]['loc_all'].ndim == 3:# such a test could be avoided if using xarrays
                        tmp = distr_cdf(
                ...
                transf_inputs[var_targ][scen][i_run,...][...,ind_NoNaN] = ss.norm.ppf(q=tmp, loc=0, scale=1)
                
            # Setting unlikely values to the threshold
            if np.any( (transf_inputs[var_targ][scen] > threshold_sigma) )  or  np.any( (transf_inputs[var_targ][scen] < -threshold_sigma) ):
                test_unlikely_values = True
                transf_inputs[var_targ][scen][np.where( transf_inputs[var_targ][scen] > threshold_sigma )] = threshold_sigma
                transf_inputs[var_targ][scen][np.where( transf_inputs[var_targ][scen] < -threshold_sigma )] = -threshold_sigma
        if test_unlikely_values:
            print('WARNING: some transformed values of '+var_targ+' are very unlikely, a possible cause is a fit missing strong signals. Action taken: blocking them at a limit.')
...
class distrib_cov:
    ...
    * mu = loc0 + delta1 / (1 + exp(beta1 * (cov_loc1-epsilon1))) + delta2 / (1 + exp(beta2 * (cov_loc2-epsilon2)))
    ...
    #--------------------
    # INITIALIZATION
    def __init__(self, data, cov_preds, distrib, fg_shape=-0.25, prior_shape=0, \
                 boundaries_params={}, boundaries_coeffs={}, \
                 option_silent=True, method_fit="Nelder-Mead", xtol_req=1e-3, maxiter=50000, maxfev=50000, error_failedfit=False ):
        ...
        elif distrib in ['GEV']: #  or (self.transfo[0] and str.split(distrib,'-')[0] in ['GEV'])
        ...
        if len(boundaries_params) == 0: # default
            if self.distrib in ['gaussian']:
                ...
                self.boundaries_params = {'loc':[-np.inf, np.inf], 'scale':[0,np.inf], 'shape':[-np.inf, 1/3]}
        ...
        if method_fit in ['CG', 'BFGS', 'L-BFGS-B']:# TNC and trust-constr: better to use xtol
            self.name_xtol = 'gtol'
        elif method_fit in ['Newton-CG', 'Powell', 'TNC', 'trust-constr']:
            self.name_xtol = 'xtol'
        elif method_fit in ['Nelder-Mead']:
            self.name_xtol = 'xatol'
        elif method_fit in ['dogleg', 'trust-ncg', 'trust-krylov', 'trust-exact', 'COBYLA', 'SLSQP']:
    ...
    def find_fg(self):
        ...
        self.dd_mean = np.mean(data_det)# np.mean(data) #np.mean(data_det) #### why did i switch from data_det to data?
        ...
        if self.distrib in [ 'GEV' ]:
            ...
            if False:
                ...
                for self.eval_fg0_with_dd_mean in [True, False]:
                    ...
                    test = self._test_coeffs(x0) * self._test_evol_params(self.data_tmp, args)
                    checks[self.eval_fg0_with_dd_mean] = [np.copy(x0), test, self.neg_loglike( x0 )]
                del self.data_tmp
                ...
                if valid_True and valid_False:# both options are valid
                    if self.eval_fg0_best:# default mode. Most of the time, will be here, taking the best first guess among the 2 options. For difficult fits, the 2nd may actually work better.
            ...
            x0[self.coeffs_names.index('loc_0')] += self.dd_mean # the location computed out of this step is NOT a proper location_0!
            if self.transfo[0]:
            ...
            x0[self.coeffs_names.index('loc_0')] -= self.dd_var # the location computed out of this step is NOT a proper location_0!
            x0[self.coeffs_names.index('mu_0')] += self.dd_var # the location computed out of this step is NOT a proper location_0!
            if self.transfo[0]:
                x0 = x0 + list(self.tmp_sol['transfo'])
            args = self._parse_args( x0 )
            if np.min(args[1]) < 0:
    ...
    def _parse_args(self, args):
        ...
        for typ in self.cov['params']:
            ...
            tmp[typ] = coeffs[self.coeffs_names.index(typ+'_0')] * np.ones(self.data.shape)
            ...
            for ii in np.arange(len(self.cov['cov_'+typ+'_names'])):
                ...
                if self.cov['cov_'+typ+'_form'][ii] == 'linear':
                    ...
                    tmp[typ] += coeffs[self.coeffs_names.index(typ+'_linear_'+name_cov)] * self.cov['cov_'+typ+'_data'][ii]
                    ...
                    tmp[typ] += coeffs[self.coeffs_names.index(typ+'_power'+str(pwr)+'_'+name_cov)] * self.cov['cov_'+typ+'_data'][ii] ** pwr
        ...
        if self._test_coeffs(args):
            ...
            if self._test_evol_params(data_fit, p_args):
                ...
                if self.distrib in ['GEV']:
                    ...
                    ll = ss.genextreme.logpdf(data_fit, loc=p_args[0], scale=p_args[1], c=p_args[2] ).sum()
                    out = ll + prior
    ...
    def fit(self):
        ...
        if np.any(np.isnan(self.data)):
            ...
            m = minimize(
                ...
                x0=self.x0,
                method=self.method_fit,
                options={"maxfev": self.maxfev, "maxiter": self.maxiter, self.name_xtol: self.xtol_req},
            )

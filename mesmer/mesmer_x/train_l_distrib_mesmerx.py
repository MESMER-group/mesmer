...
def xr_train_distrib(predictors, target, target_name, expr, expr_name, option_2ndfit=True, r_gasparicohn_2ndfit=500, scores_fit=['func_optim', 'NLL', 'BIC']):
    """
    ...
    predictors_np = {inp:listxrds_to_np(listds=predictors, name_var=inp, forcescen=list_scens) for inp in expression_fit.inputs_list}
    ...
    for coef in expression_fit.coefficients_list:
        coefficients_xr[coef] = xr.DataArray( np.nan, coords={'gridpoint': gridpoints}, dims=('gridpoint',) )
    quality_xr = xr.Dataset()
    for score in scores_fit:
        quality_xr[score] = xr.DataArray( np.nan, coords={'gridpoint': gridpoints}, dims=('gridpoint',) )
    #--------------------------------------------------------------------------------
    #--------------------------------------------------------------------------------
    ...
    for igp, gp in enumerate(gridpoints):
        print( 'Fitting the variable '+target_name+' with the expression ' + expr + ': ' + str(np.round(100.*(igp+1)/gridpoints.size,1)) + '%', end='\r' )
        
        # shaping target for this gridpoint
        target_np = listxrds_to_np(listds=target, name_var=target_name, forcescen=list_scens, coords={'gridpoint':gp})
            
        # training
        coefficients_np, quality_np = np_train_distrib(targ_np=target_np, pred_np=predictors_np, expr_fit=expression_fit, scores_fit=scores_fit)
    ...
    if option_2ndfit:
        ...
        corr_gc = gaspari_cohn_correlation_matrices(geodist, [r_gasparicohn_2ndfit])[r_gasparicohn_2ndfit]
        ind_nonan = np.where(np.isnan(coefficients_xr[expression_fit.coefficients_list[0]])==False)[0]

        for igp, gp in enumerate(gridpoints):
            print( 'Fitting the variable '+target_name+' with the expression ' + expr + ' (2nd round): ' + str(np.round(100.*(igp+1)/gridpoints.size,1)) + '%', end='\r' )
            ...
            for ic, coef in enumerate(expression_fit.coefficients_list):
                fg[ic] = weighted_median(data=coefficients_xr[coef].values[ind_nonan], weights=corr_gc[igp,ind_nonan])
        
            # shaping target for this gridpoint
            target_np = listxrds_to_np(listds=target, name_var=target_name, forcescen=list_scens, coords={'gridpoint':gp})
                
            # training
            coefficients_np, quality_np = np_train_distrib(targ_np=target_np, pred_np=predictors_np, expr_fit=expression_fit, scores_fit=scores_fit, first_guess=fg)
...
def np_train_distrib(targ_np, pred_np, expr_fit, scores_fit=['func_optim', 'NLL', 'BIC'], first_guess=None):
    self = distrib_cov(data_targ=targ_np, data_pred=pred_np, expr_fit=expr_fit, scores_fit=scores_fit, first_guess=first_guess)
    self.fit()
...
class distrib_cov:
    ...
    def __init__(self, data_targ, data_pred, expr_fit, data_targ_addtest=None, data_preds_addtest=None, threshold_min_proba=1.e-9, boundaries_params=None, boundaries_coeffs=None,\
                 first_guess=None, func_first_guess=None, scores_fit=['func_optim', 'NLL', 'BIC'], options_optim=None, options_solver=None):
        # preparing basic information
        self.data_targ = data_targ
        self.n_sample = len(self.data_targ) # can be different from length of predictors IF no predictors.
        ...
        if np.any([np.any(np.isnan(self.data_pred[pred])) for pred in self.data_pred]) or np.any([np.any(np.isinf(self.data_pred[pred])) for pred in self.data_pred]):
            raise Exception('NaN or infinite values in predictors of fit')
        self.expr_fit = expr_fit
        
        # preparing additional data
        self.add_test = (data_targ_addtest is not None) and (data_preds_addtest is not None)
        if (self.add_test == False) and ((data_targ_addtest is not None) or (data_preds_addtest is not None)):
            raise Exception("Only one of data_targ_addtest & data_preds_addtest have been provided, not both of them. Please correct.")
        ...
        if boundaries_params is not None:
            for param in boundaries_params:
                self.boundaries_params[param] = [np.max([boundaries_params[param][0],self.boundaries_params[param][0]]), \
                                                 np.min([boundaries_params[param][1],self.boundaries_params[param][1]])]
        ...
        if len(self.first_guess) != self.n_coeffs:
            raise Exception("The provided first guess does not have the correct shape: (" + str(self.n_coeffs) + ")")
        self.scores_fit = scores_fit

        # preparing information on solver
        default_options_solver = dict(method_fit="Powell", xtol_req=1e-6, ftol_req=1.e-6, maxiter=1000 * self.n_coeffs*np.log(self.n_coeffs), maxfev=1000 * self.n_coeffs*np.log(self.n_coeffs), error_failedfit=False, fg_with_global_opti=False)
        ...
        if self.method_fit in ['dogleg', 'trust-ncg', 'trust-krylov', 'trust-exact', 'COBYLA', 'SLSQP'] + ['CG', 'Newton-CG']:
            raise Exception('method for this fit not prepared, to avoid')
        else:
            self.name_xtol = {'BFGS':'xrtol', 'L-BFGS-B':'gtol', 'Nelder-Mead':'xatol', 'Powell':'xtol', 'TNC':'xtol', 'trust-constr':'xtol'}[ self.method_fit ]
            self.name_ftol = {'BFGS':'gtol', 'L-BFGS-B':'ftol', 'Nelder-Mead':'fatol', 'Powell':'ftol', 'TNC':'ftol', 'trust-constr':'gtol'}[ self.method_fit ]
        ...
        default_options_optim = dict(weighted_NLL=False, type_fun_optim='NLL', threshold_stopping_rule=None, exclude_trigger=None, ind_year_thres=None)
        ...
        if (self.type_fun_optim == 'NLL') and ((self.threshold_stopping_rule is not None) or (self.ind_year_thres is not None)) or\
(self.type_fun_optim == 'fcNLL') and ((self.threshold_stopping_rule is None) or (self.ind_year_thres is None)):
            raise Exception("Lack of consistency on the options 'type_fun_optim', 'threshold_stopping_rule' and 'ind_year_thres', not sure if the stopping rule will be employed")
    ...
    def eval_weights(self, n_bins_density=40):
        if self.weighted_NLL:
            if len(self.data_pred) == 0:
                ...
                gmt_hist, tmpb = np.histogramdd( sample=tmp, bins=[np.linspace((m1-0.05*(m2-m1))[i], (m2+0.05*(m2-m1))[i], n_bins_density) for i in range(n_preds)] )
                gmt_bins_center = [0.5 * (tmpb[i][1:] + tmpb[i][:-1]) for i in range(n_preds)]
    ...
    def _test_coeffs(self, values_coeffs):
        ...
        for coeff in self.boundaries_coeffs:
            ...
            if coeff not in self.expr_fit.list_coefficients:
                raise Exception("Provided wrong boundaries on coefficient, " + coeff + " does not exist in expr_fit")
            else:
    ...
    def _test_evol_params(self, distrib, data):
        ...
        for param in self.boundaries_params:
            ...
            if np.any(self.expr_fit.parameters_values[param] < bottom) or np.any(top < self.expr_fit.parameters_values[param]):
                test = False # out of boundaries
        ...
        if test: # dont try testing if there are issues on the parameters
            ...
            if np.any(np.isnan(bottom)) or np.any(np.isnan(top)) or np.any(data < bottom) or np.any(top < data):
    ...
    def _test_proba_value(self, distrib, data):
        ...
        return np.all( 1 - cdf >= self.threshold_min_proba ) * np.all( cdf >= self.threshold_min_proba )
    ...
    def test_all(self, coefficients):
        ...
        if test_coeff == False:
            ...
            if self.add_test:
                test_param = self._test_evol_params(distrib, self.data_targ) * self._test_evol_params(distrib_add, self.data_targ_addtest)
            else:
            ...
            if test_param == False:
                ...
                if self.threshold_min_proba is None:
                    ...
                    if self.add_test:
                        test_proba = self._test_proba_value(distrib, self.data_targ) * self._test_proba_value(distrib_add, self.data_targ_addtest)
                    else:
    ...
    def find_fg(self):
        ...
        with warnings.catch_warnings():
            ...
            self.smooth_data_targ = self.smooth_data(self.data_targ) # facilitates the representation of the trends
            ...
            pred_high = {p:np.mean(self.data_pred[p][ind_targ_high]) for p in self.data_pred}
            deriv_targ = {p: (np.mean(self.smooth_data_targ[ind_targ_high]) - np.mean(self.smooth_data_targ[ind_targ_low])) / (pred_high[p] - pred_low[p]) for p in self.data_pred}
            self.fg_info_derivatives = {'pred_low':pred_low, 'pred_high':pred_high, 'deriv_targ':deriv_targ, 'm':m}
            ...
            if self.first_guess is None:
                ...
                globalfit_d01 = basinhopping(func=self.fg_fun_deriv01, x0=self.fg_coeffs, niter=10 ) # warning, basinhopping tends to indroduce non-reproductibility in fits, reduced when using 2nd round of fits
            ...
            self.fg_ind_loc = np.array([self.expr_fit.coefficients_list.index(c) for c in self.expr_fit.coefficients_dict['loc']])
            localfit_loc = self.minimize(func=self.fg_fun_loc, x0=self.fg_coeffs[self.fg_ind_loc], fact_maxfev_iter=len(self.fg_ind_loc)/self.n_coeffs, option_NelderMead='best_run' )
            ...
            self.fg_ind_sca = np.array([self.expr_fit.coefficients_list.index(c) for c in self.expr_fit.coefficients_dict['scale']])
            if self.first_guess is None:
                x0 = np.std(self.data_targ) * np.ones(len(self.expr_fit.coefficients_dict['scale'])) # compared to all 0, better for ref level but worse for trend
            else:
                x0 = self.fg_coeffs[self.fg_ind_sca]
            localfit_sca = self.minimize(func=self.fg_fun_sca, x0=x0, fact_maxfev_iter=len(self.fg_ind_sca)/self.n_coeffs, option_NelderMead='best_run' )
            ...
            localfit_nll = self.minimize(func=self.fg_fun_NLL_notests, x0=self.fg_coeffs, fact_maxfev_iter=1, option_NelderMead='best_run' )
            ...
            localfit_opti = self.minimize(func=fun_opti_prob, x0=self.fg_coeffs, fact_maxfev_iter=1, option_NelderMead='best_run' )
            ...
            if self.fg_with_global_opti:
                ...
                for i_c in np.arange( n_coeffs ):
                    vals_bounds = self.find_bound(i_c=i_c, x0=self.fg_coeffs, fact_coeff=-0.05), self.find_bound(i_c=i_c, x0=self.fg_coeffs, fact_coeff=0.05)
                ...
                globalfit_all = shgo(self.func_optim, bounds, sampling_method='sobol')# sobol or halton, observed lower performances with simplicial. n=1000, options={'maxiter':10000, 'maxev':10000})
    ...
    def minimize(self, func, x0, fact_maxfev_iter=1, option_NelderMead='dont_run'):
        ...
        """
        fit = minimize(func, x0=x0, method=self.method_fit,\
                       options={"maxfev": self.maxfev*fact_maxfev_iter, "maxiter": self.maxfev*fact_maxfev_iter, self.name_xtol: self.xtol_req, self.name_ftol: self.ftol_req})
        ...
        if (option_NelderMead=='fail_run' and fit.success == False) or (option_NelderMead=='best_run'):
            fit_NM = minimize(func, x0=x0, method='Nelder-Mead',\
                              options={"maxfev": self.maxfev*fact_maxfev_iter, "maxiter": self.maxiter*fact_maxfev_iter, 'xatol': self.xtol_req, 'fatol': self.ftol_req} )
            if (option_NelderMead=='fail_run') or (option_NelderMead=='best_run' and fit_NM.fun < fit.fun):
                fit = fit_NM
        return fit
    ...
    def fg_fun_deriv01(self, x):
        ...
        deriv = {p: (loc_high - loc_low) / (self.fg_info_derivatives['pred_high'][p] - self.fg_info_derivatives['pred_low'][p]) for p in self.data_pred}
        return np.sum([(deriv[p] - self.fg_info_derivatives['deriv_targ'][p])**2 for p in self.data_pred]) + (0.5*(loc_low+loc_high) - self.fg_info_derivatives['m'])**2
    ...
    def fg_fun_sca(self, x_sca):
        ...
        distrib = self.expr_fit.evaluate(x, self.data_pred)
        loc = self.expr_fit.parameters_values['loc']
        sca = self.expr_fit.parameters_values['scale'] # better to use that one instead of deviation, which is affected by the scale
    ...
    def bic(self, distrib):
        return self.n_coeffs * np.log(self.n_sample) / self.n_sample - 2 * self.loglike(distrib)
    ...
    def crps(self, coeffs):
        ...
        for i in np.arange(tmp_fit.n_sample):
            distrib = tmp_fit.expr_fit.evaluate(coeffs, {p:tmp_fit.data_pred[p][i] for p in tmp_fit.data_pred})
            tmp_cprs.append( ps.crps_quadrature(x=tmp_fit.data_targ[i], cdf_or_dist=distrib, xmin=-10*np.abs(tmp_fit.data_targ[i]), xmax=10*np.abs(tmp_fit.data_targ[i]), tol=1.e-4) )
    ...
    def fit(self):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            m = self.minimize(func=self.func_optim, x0=self.fg_coeffs, fact_maxfev_iter=1, option_NelderMead='best_run' )

...
def extract_time_lon_lat_wgt3d(data):
    ...
    # extract time
    time = data.time.values # years already extracted in 'update_time_axis', due to additional time formats introduced in other archives.
...
def find_files_cmip(esm, var, scenario, cfg):
    ...
    elif var in ['txx', 'mrso', 'mrsomean', 'mrso_minmon', 'fwixx', 'fwixd', 'fwisa', 'fwils']:
        # styles for search
...
def load_cmip(targ, esm, scen, cfg):
    ...
    if scen[:2] == "h-":
        ...
        raise Exception("No version without historical time period is currently implemented. Crashing this function.")
    ...
    for run_path in path_runs_list:
        ...
        data = data.sel(
            time=
            slice(max([data.time.values[0],float(cfg.time["start"])]),
        ...
        if targ not in ['mrso', 'mrsomean', 'mrso_minmon', 'fwixx', 'fwixd', 'fwisa', 'fwils']:
            refs[run] = [ data[var_in_file].sel(time=slice(cfg.ref["start"], cfg.ref["end"])).mean(dim="time").values ]
        else:
            refs[run] = [ data[var_in_file].sel(time=slice(cfg.ref["start"], cfg.ref["end"])).mean(dim="time").values,
                          data[var_in_file].sel(time=slice(cfg.ref["start"], cfg.ref["end"])).std(dim="time").values ]
        
    ## preparing references
    if cfg.ref["type"] == "all":## averaging the reference over all
        var_ref = [ sum( [refs[run_path][i]  for run_path in path_runs_list] ) / len(path_runs_list) for i in range(refs[run]) ]
    elif cfg.ref["type"] == "first":## choosing only the first one
        var_ref = refs["1"]
    elif cfg.ref["type"] in ['individ', 'none']:## no need to calculate a global reference
        pass

    ## computing anomalies
    for run in var.keys():
        if targ in ['mrso', 'mrsomean', 'mrso_minmon', 'fwixx', 'fwixd', 'fwisa', 'fwils']:
    ...
    for run_path in path_runs_list:
        ...
        if targ in ['hfds']:## specific case
            masked_var = np.ma.masked_array( var[run], np.isnan(var[run]) ) ## account for missing values over land
            GVAR[run] = np.average(masked_var, axis=(1, 2), weights=wgt3d) ## no need to keep mask since False everywhere
        else:
...
def load_cmip_file(run_path, scen, cfg):
    ...
    if cfg.gen == 5:
        ...
        if "ssp534-over" in run_path:
            ...
            ## update here the time axis of the files for more robust combine_first, due to different time formats.
            ssp534over.coords['time'] = update_time_axis( ssp534over.time.values, run_path )
            ssp585.coords['time'] = update_time_axis( ssp585.time.values, run_path.replace(scen, "ssp585") )
            hh.coords['time'] = update_time_axis( hh.time.values, run_path.replace(scen, "historical") )
            
            #### Order of scenarios: historical (1850-2014) - ssp585 (2015-2039) - ssp534-over (2040-...)
            data = ssp534over.combine_first( ssp585.sel(time=slice(None, "2039")).combine_first( hh ) )
...
def update_time_axis( time_axis, run_path ):
    ...
    for val in time_axis:
        if type(val) in [pd._libs.tslibs.timestamps.Timestamp, cft._cftime.DatetimeGregorian, cft._cftime.DatetimeNoLeap, cft.Datetime360Day, cft.DatetimeProlepticGregorian]:
            yr,mon,day = val.year,val.month,val.day ## nothing to do, can keep "val" this way
        elif type(val) in [np.datetime64]:
            val = pd.DatetimeIndex( [val] )
            yr,mon,day = val[0].year,val[0].month,val[0].day
        else:
            raise Exception("This time format ("+str(type(val))+") is not prepared yet, but appears in: "+run_path)
        
        if np.all(np.isnan([yr,mon,day])): ## some of cmip5_X files have np.datetime64 'NaT' instead of a valid format, but values are still provided by the ESM. To exclude?
            tm.append( [tm[-1][0]+1, tm[-1][1], tm[-1][2]] ) ## adding last year+1, same month and day.
        else:
            tm.append( [yr,mon,day] )
    if np.any( np.diff(np.array(tm)[:,1])!=0. )  or  np.any( np.diff(np.array(tm)[:,2])!=0. ):
...
def test_combination_vars( list_vars, esm, scen, cfg ):
    ...
    dico_runs = {targ:[str.split(run,'_')[-2] for run in dico_paths[targ]] for targ in list_vars}

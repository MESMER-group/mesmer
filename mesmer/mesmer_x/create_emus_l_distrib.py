...
def backtransf_normal2distrib(transf_emus_lv, preds, params_distrib, force_scen=None):
    ...
    for var_targ in transf_emus_lv["all"].keys():
        ...
        ind_NoNaN = [
            ii
            for ii in range(nr_gps)
            if ii not in params_distrib[var_targ]["gridpoints_nan"]
        ]
        ...
        descrip_fit = read_form_fit_distrib(
            params_distrib[var_targ]["form_fit_distrib"]
        )

        # preparing outputs:
        for scen in list_scens:
            backtransf_emus[var_targ][scen] = np.nan * np.ones(
                transf_emus_lv["all"][var_targ].shape
            )
        ...
        for i_emu in np.arange(
            ...
            print(
                ...
                var_targ,
                ": ",
                str(np.round(100.0 * (i_emu + 1) / nr_emus, 1)),
                "%",
                end="\r",
            )
            ...
            ).T  # can be run once for all scenarios
            ...
            for scen in list_scens:
                ...
                if distr in ["GEV"]:
                    if (
                        ...
                        params_all[scen]["loc_all"].ndim == 2
                ...
                if "transfo" in descrip_fit.keys():
                    left = params_distrib[var_targ]["parameters"]["transfo_asymptleft"][
                        ind_NoNaN
                    ]
                    right = params_distrib[var_targ]["parameters"][
                        "transfo_asymptright"
                    ][ind_NoNaN]
                    transfo = descrip_fit["transfo"]
                    if transfo in ["generalized_logistic", "generalized_algebraic"]:
                        alpha = params_distrib[var_targ]["parameters"]["transfo_alpha"][
                            ind_NoNaN
                        ]
                        backtransf_emus[var_targ][scen][i_emu, ...] = (
                            sigmoid_backtransf(
                                data=data_backPIT,
                                left=left,
                                right=right,
                        ...
                        backtransf_emus[var_targ][scen][i_emu, :, ind_NoNaN] = (
                            sigmoid_backtransf(
                                data=data_backPIT,
                                left=left,
                                right=right,

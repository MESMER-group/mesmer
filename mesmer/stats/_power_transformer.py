...
class YeoJohnsonTransformer:
    ...
    def lambda_function(coeffs: np.ndarray, local_yearly_T: np.ndarray) -> np.ndarray:
        ...
        coeffs : ndarray of shape (n,)
            Coefficients for the covariate function.
        local_yearly_T : ndarray of shape (n_years,)
            Yearly values of one gridcell and month used as predictor
            for lambda.
    ...
    def _optimize_lambda_np(self, monthly_residuals, yearly_pred):
    ...
    def get_lambdas_from_covariates(
        ...
        *,
        ...
        if time_coords is not None:
            ...
            lambdas = lambdas.assign_coords(
                {time_dim: (sample_dim, time_coords.values)}
            )
    ...
    def fit(
        ...
        _check_dataarray_form(
            yearly_pred, name="yearly_pred", required_dims=monthly_dims
        )
    ...
    def transform(
        ...
        lambdas = self.get_lambdas_from_covariates(
            lambda_coeffs,
            yearly_pred,
            time_dim=time_dim,

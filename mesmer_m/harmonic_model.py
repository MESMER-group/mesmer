...
def generate_fourier_series_np(coeffs, n, x, mon):
    ...
    return sum( [(coeffs[idx]*x + coeffs[idx+1])*np.sin(np.pi*i*(mon%12+1)/6)+(coeffs[idx+2]*x+coeffs[idx+3])*np.cos(np.pi*i*(mon%12+1)/6) for i,idx in enumerate(np.arange(n*4, step = 4))] )
...
def fit_fourier_series_np(x, y, n, repeat=False):
    ...
    if repeat:
        x_train = np.repeat(x, 12) ## each month scales to the yearly value at that timestep so need to repeat
                               ## yearly temp. array for fitting if not already done
    else:
        x_train = x
        
    mon_train = np.tile(np.arange(1,13),int(x.shape[0]/12)) ## also get monthly values
    mon_train = (np.pi*(mon_train%12+1))/6 ##for simplicity's sake we take month values in there harmonic form
    
    ## construct predictor matrix
    
    
    A = np.hstack(([np.array([x_train*np.sin(i_n*mon_train),np.sin(i_n*mon_train),
                                      x_train*np.cos(i_n*mon_train),np.sin(i_n*mon_train)]).T for i_n in range(n)]))
...
def fit_to_bic_xr(X, Y, max_n):
    ...
    n_sel, coeffs, preds = xr.apply_ufunc(
        ...
        kwargs={"max_n": max_n, "repeat" : True},
    )

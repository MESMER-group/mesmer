...
def dummy_harmonic_data(nr_runs, n = 2, x = None):
    ...
    mon = np.tile(np.arange(1,13),x.shape[0]) ## generate month values for length of yearly data
    x = np.repeat(x,12) ## expand yearly data so each month has its corresponding yearly temperature
...
def dummy_fit(nr_runs, n = 3, fit_to_bic = False):
    ...
    x, coeffs_org = dummy_harmonic_data(nr_runs, n)
    
    if fit_to_bic:
        
        return x, y, coeffs_org, hm.fit_to_bic_np(x[::12], y, 8)
    
    else:
        
        return x, y, coeffs_org, hm.fit_fourier_series_np(x[::12], y, n)
...
def BIC_gridsearch_domain(max_n,max_nr_runs):
    ...
    for i_nr in range(1,max_nr_runs+1):
        ...
        for i_n in range(1,max_n+1):
            ...
            mse[i_n-1,i_nr-1,:] = np.mean((y.reshape(-1,12)-y_preds[i_nr][i_n,:].reshape(-1,12))**2, axis = 0)
...
def check_fit_to_bic_xr(nr_runs, cells , max_n):
    ...
    times_mon = np.arange('1870-01', '2101-01',np.timedelta64(1,'M'), dtype='datetime64')

    X = xr.DataArray(np.hstack(([np.repeat(dummy_yearly_trend(nr_runs),12).reshape(-1,1) for i_cell in np.arange(cells)])), dims=["time",  "cell"], coords={'time': np.tile(times_mon,nr_runs)})

    Y = xr.DataArray(np.hstack(([dummy_harmonic_data(nr_runs, x = X.values[::12,i_cell])[2].reshape(-1,1) for i_cell in np.arange(cells)])), dims=["time",  "cell"], coords={'time': np.tile(times_mon,nr_runs)})

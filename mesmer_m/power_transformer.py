...
class PowerTransformerVariableLambda(PowerTransformer):
    ...
    **kwargs : 
    ...
    def fit_fmin(self, X, X_func, n_index):
        ...
        self.coeffs_ = np.array(Parallel(n_jobs=-1,verbose=False)(delayed(self._yeo_johnson_optimize_fmin)(X[:,i_grid], X_func[:,i_grid]) for i_grid in np.arange(n_index)))#,  desc = 'Optimizing Fmin:', position = 0, leave = True, file=sys.stdout)))
    ...
    def _yeo_johnson_optimize_fmin(self, x, x_func):
        ...
        return minimize(_neg_log_likelihood, np.array([0.01,0.01]), bounds=bounds, method='SLSQP',jac=rosen_der
                        ).x
